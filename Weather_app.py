...
# main function that handles the greeting and code flow using try blocks in if statements
...
def Main():
    ...
    '''
    while True:
        userInput = input('Would you like to enter your City or your Zip Code? Please enter either *city* or *zip*: ')
        ...
        if userInput == 'city':
            try:
                ...
                print('\nI am not familiar with that city, please try again.\n')
                City()
        elif userInput == 'zip':
            try:
                print('\nOne moment while I pull that up.\n')
                ...
                print('\nI am not familiar with that zip code, please try again.\n')
                Zip()
        else:
            print('\nI dont understand, please try again.\n')

           **** MAKE THIS PART NOT STUPID. REMOVE TRY BLOCKS THAT DONT REALLY DO ANYTHING ANYWAYS.REPLACE WITH JUST IF ELSE'S ****
...
def Greeting(): # Generic greeting message to greet the user. 
    ...
    print('Welcome to my new Weather Application. Please follow the prompts to see your forecast.\n')
...
def City(): # Handles the API call if the City option is chosen in Main
...
def Zip(): # Handles the API call if the Zip Code option is chosen in Main
...
def Weather(forecast): # stores, formats and displays the json data from both City and Zip API calls
    #print(forecast)
  ...
  # Rounding results into round numbers where necessary
    ...
    if winddir >= 350 or winddir >=0 and winddir < 20: # List of IF's that convert wind direction from degrees to human readable cardinal directions.
        winddir = 'North'
